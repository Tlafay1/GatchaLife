      ...
      <Card v-for="style in styles" :key="style.id">
        <CardHeader class="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <div class="flex gap-2">
            ...
            <Button variant="ghost" size="icon" class="text-destructive hover:text-destructive" @click="handleDelete(style.id!)">
        ...
        <CardContent>
          <div class="space-y-2 text-sm">
            ...
            <div>
              ...
              <div class="bg-muted p-2 rounded text-xs font-mono break-all">{{ style.style_keywords }}</div>
            </div>
            <div v-if="style.composition_hint">
              <span class="text-muted-foreground block mb-1">Composition Hint:</span>
              <div class="bg-muted p-2 rounded text-xs font-mono break-all text-blue-400/80">{{ style.composition_hint }}</div>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <DialogContent class="sm:max-w-[500px]">
        ...
        <div class="grid gap-4 py-4">
          ...
          <div class="grid gap-2">
            ...
            <Textarea 
              v-model="form.style_keywords" 
              placeholder="e.g. anime style, cel shaded, vibrant colors" 
              class="h-24"
            />
          ...
          <div class="grid gap-2">
            ...
            <Textarea 
              v-model="form.composition_hint" 
              placeholder="e.g. full body shot, dynamic pose" 
            />
          </div>
        </div>
